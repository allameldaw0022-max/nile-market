...
> **D18 + D31:** قيم الحدود **لا تُخترع**. `configured_at = null` تعني
> «لم تُضبط بعد» (الحد لا يُنفَّذ)، وليست «بلا حد بقرار». Admin يضبط كل
> مفتاح صراحةً إلى رقم أو «بلا حد».
> **بوابة الإطلاق التجاري:** `commercial_launch_enabled` لا يُفعَّل ما
> لم تكتمل أسعار وحدود الباقات الأساسية — وتُعرض حالة «Plan
> Configuration غير مكتمل» في `/admin` حتى ذلك الحين.
