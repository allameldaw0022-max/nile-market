...
2. القديم يبقى بحالة `active` مع `redirect_to_primary` ⇒ **301 لمدة 12
   شهرًا** (`reserved_slugs.reserved_until`). بعدها يتوقف التوجيه
   ويُحرَّر الاسم لمتجر آخر — **لا وعد بـ301 دائمة** (D33).
3. خلال فترة الحجز لا يستطيع أي متجر آخر أخذ الاسم ⇒ منع Takeover
   واستغلال الروابط القديمة.
4. واجهة التاجر تذكر المدة صراحةً قبل تأكيد التغيير: «الروابط القديمة
   ستعمل 12 شهرًا من تاريخ التغيير».
5. لا تُفقد أي بيانات؛ وتُبطَّل الـcaches ويُحدَّث الـsitemap.
6. يُسجَّل التغيير في `audit_logs` ويتطلب `settings:update`.
