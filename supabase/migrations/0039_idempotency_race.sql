-- =====================================================================
-- 0039 التكرار تحت التزامن يعيد النتيجة لا خطأً (إضافية)
--
-- ★ الثغرة: كل الدوال ذات مفتاح التكرار تتبع نمط «اقرأ ثم أدرج»:
--
--     select ... where idempotency_key = v_key;   -- لم يوجد
--     ...
--     insert into ... values (..., v_key);        -- ⚡ سباق هنا
--
-- بين القراءة والإدراج نافذة. أُثبت بالتنفيذ: خمس جلسات متزامنة بنفس
-- المفتاح ⇒ طلب واحد فقط يُنشأ (الفهرس الفريد يحمي البيانات) لكن
-- **جلستين تلقّتا خطأ Postgres خامًا**:
--
--     duplicate key value violates unique constraint
--
-- الأثر الحقيقي: الزبون ينقر «تأكيد الطلب» مرّتين، أو تعيد الشبكة
-- الإرسال. الطلب يُنشأ فعلًا، لكن النداء الثاني يعود بخطأ فتعرض
-- الواجهة «فشل الطلب». فيطلب الزبون من جديد بمفتاح جديد — ويصير
-- لدينا **طلبان حقيقيان**. أي أن ضمانة التكرار كانت تحمي القاعدة
-- ولا تحمي المستخدم.
--
-- الإصلاح: التقاط `unique_violation` عند الإدراج وإعادة الصف القائم —
-- وهو تمام نمط Idempotency لا نصفه.
-- =====================================================================

-- ---------------------------------------------------------------------
-- الطلبات
-- ---------------------------------------------------------------------
create or replace function app.order_by_key(p_store_id uuid, p_key text)
...
as $$
  select * from public.orders
   where store_id = p_store_id and idempotency_key = p_key;
$$;

comment on function app.order_by_key(uuid, text) is
  'قراءة الطلب بمفتاح التكرار — تُستخدم في معالج السباق داخل create_order.';
...
begin
  ...
  -- ★ السباق: جلسة أخرى بنفس المفتاح قد تكون أدرجت بين قراءتنا وهذا
  -- الإدراج. الفهرس الفريد يمنع الازدواج، ونحن نعيد الطلب القائم بدل
  -- أن نرمي خطأ قاعدة خامًا في وجه زبون نقر مرّتين.
  begin
  ...
  exception when unique_violation then
    v_existing := app.order_by_key(p_store_id, v_key);
    return query select v_existing.id, v_existing.order_number,
                        v_existing.total, v_existing.guest_token;
    return;
  end;
